...
class MM_GCN(nn.Module):
    ...
    def forward(self, a, v, l, dia_len, qmask, use_residue,test_label=False,tag=None,p_c_a=None,p_c_v=None,p_c_t=None,padding_policy_a=None,padding_policy_v=None,padding_policy_l=None):
        #(1473,2)
        ...
        adj = self.create_big_adj(a, v, l, dia_len,p_c_a,p_c_v,p_c_t, self.modals, self.modal_weight,policy,tag)
    ...
    def create_big_adj(self, a, v, l, dia_len,p_c_a,p_c_v,p_c_t, modals, modal_weight=1.0,policy=None,tag=None):
        ...
        for i in range(len(dia_len)):
            ...
            for j, x in enumerate(features):
                if j < 0:
                ...
                elif tag=="acouf" and j==0:
                    sub_adj = torch.zeros((dia_len[i], dia_len[i])) + torch.eye(dia_len[i])
                elif tag=="visuf" and j==1:
                    sub_adj = torch.zeros((dia_len[i], dia_len[i])) + torch.eye(dia_len[i]) 
                elif tag=="textf" and j==2:
                    sub_adj = torch.zeros((dia_len[i], dia_len[i])) + torch.eye(dia_len[i])   
                else:
                    if p_c[j]==None:
                        mask_m=torch.ones((dia_len[i], dia_len[i])).cuda()
                    else:
                        mask_m=torch.ones((dia_len[i], dia_len[i])).cuda()
                        for z in p_c[j]:
                            if z[1]==i:
                    ...
                    sub_adj[:dia_len[i], :dia_len[i]] = sim_matrix*mask_m*mask
        ...
        if policy is not None:
            policy=(policy<0.3).nonzero(as_tuple=True)
