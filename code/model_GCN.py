...
class GCNLayer1(nn.Module):
    ...
    def forward(self, inputs, dia_len, topicLabel,qmask):
        if self.new_graph:
            # pdb.set_trace()
            adj = self.message_passing_directed_speaker(inputs, dia_len, qmask).cuda()
        else:
            adj = self.message_passing_wo_speaker(inputs, dia_len, topicLabel).cuda()
    ...
    def message_passing_directed_speaker(self, x, dia_len, qmask):
        ...
        for (i, len_) in enumerate(dia_len):
            speaker0 = []
            speaker1 = []
            for (j, speaker) in enumerate(qmask[:len_]):
                # if speaker[0] == 1:
                if speaker[0][0] == 1:
                    speaker0.append(j)
                else:
...
class GCNII_lyc(nn.Module):
    ...
    def forward(self, x, dia_len, topicLabel, use_residue=False,adj=None, test_label=False):
        if adj is None:
            if self.new_graph:
        ...
        x = F.dropout(x, self.dropout, training=self.training)
        #维度转化为100，graph_h
        layer_inner = self.act_fn(self.fcs[0](x))  # 4419,100 << 4419,200
        _layers.append(layer_inner)
        layer_inner = F.dropout(layer_inner, self.dropout, training=self.training)  # 4419,100
